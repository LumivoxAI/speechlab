...
class SileroAsyncClient(BaseZMQAsyncClient):
    def __init__(
        self,
        address: str,
        client_id: str = None,
        name: str = "SileroClient",
    ) -> None:
    ...
    async def tts(self, request: SileroRequest) -> AsyncIterator[bytes]:
        ...
        try:
            while True:
                parts = await self.recv(SileroResponse)
                if parts is None:
                    raise RuntimeError("Failed to receive response from server")
                meta: SileroResponse = parts[0]
